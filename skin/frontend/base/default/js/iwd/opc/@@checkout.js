...
IWD.OPC = {
    ...
    validatePayment: function () {
        ...
        var paymentMethodForm = new Validation('co-payment-form', {
            onSubmit: false, stopOnFirst: false, focusOnError: false, onFormValidate: function (result, form) {

                var cc_number = $j_opc(form).find("dt.active").next('dd').find("input[name='payment[cc_number]']");
                if(!cc_number && payment.currentMethod == 'braintree'){
                    cc_number = $j_opc(form).find("dt.active").next('dd').find("#braintree_cc_number");
                }
    ...
    bindChangePaymentFields: function(){
        ...
        $j_opc('#co-payment-form input').keyup(function(event){
            IWD.OPC.Checkout.paymentInProgress = true;
            ...
            IWD.OPC.Checkout.ajaxProgress = setTimeout(function(){
                IWD.OPC.Checkout.paymentInProgress = false;
                IWD.OPC.validatePayment();
            }, 800);
        });
    ...
    callSaveOrder: function(form){
        if(IWD.OPC.Checkout.paymentInProgress == false){
...
//					IWD.OPC.recheckAgree();
    ...
    reloadShippingsPayments: function(form_type, delay, force_update) {
        if(typeof(delay) == 'undefined' || delay == undefined || !delay)
            delay = 1400;
        if(typeof force_update == 'undefined' || force_update === undefined)
            force_update = false;
        ...
        IWD.OPC.Checkout.updateShippingPaymentProgress = setTimeout(function(){
            ...
            if(ft === undefined) {}
            form_type = ft;
            var form = $j_opc('#opc-address-form-'+form_type);
            var formArray = form.serializeArray();

            var requiredForShipperHQFields = form.find('input.required-entry, select.required-entry');
//                var requiredForShipperHQFields = form.find('[name="'+form_type+'[region_id]"]', '[name="'+form_type+':country_id]"]', '[name="'+form_type+':postcode]"]');
            var isAllFilled = 0;
            for(var i=0;i<requiredForShipperHQFields.length; i++) {
                if(requiredForShipperHQFields.eq(i).val() != '') {
                    isAllFilled++;
                }
            }

            formArray = IWD.OPC.Checkout.applyShippingMethod(formArray);
            ...
            if(force_update) {
                IWD.OPC.Checkout.showLoader();
                $j_opc('.opc-ajax-loader .please-wait-message').show();
            }
            if(IWD.OPC.Checkout.checkReloadShippingsPayments(form)
                || force_update === true
            )
                IWD.OPC.Checkout.xhr2 = $j_opc.post(IWD.OPC.Checkout.config.baseUrl + 'onepage/json/reloadShippingsPayments',formArray, IWD.OPC.Checkout.reloadShippingsPaymentsResponse,'json');

        }, delay);

    },
    checkReloadShippingsPayments: function (form) {
        // check if ShipperHQ is enabled
        if (typeof IWD.OPC.Checkout.config.shipperhqEnabled == 'undefined' || IWD.OPC.Checkout.config.shipperhqEnabled === false)
            return true;
        ...
        return isAllFilled > 0 && requiredForShipperHQFields.length == isAllFilled;
    },
    ...
    updateShippingMethods: function() {
        $j_opc(document).on('click', '#update-shipping-methods', function(e){
            var mode = $j_opc('[name="billing[use_for_shipping]"]').prop('checked') ? 'billing' : 'shipping';
            // add form validation
            var addressForm = new Validation('opc-address-form-'+mode, { onSubmit : false, stopOnFirst : false, focusOnError : false});
            if(addressForm.validate())
            {
                IWD.OPC.Checkout.reloadShippingsPayments(mode, 0.01 /* really small but not 0 delay */, true /* force update */);
            }
            else
            {
                // animate message
                var msg = $j_opc('.opc-required-msg');
                msg.css({'background-color': '#C5D4C5', 'color': '#022902'});
                setTimeout(function(){
...
IWD.OPC.Billing = {
    ...
    validateAddressForm: function (form) {
        ...
        var addressForm = new Validation('opc-address-form-billing', {
            onSubmit: false,
            stopOnFirst: false,
            focusOnError: false
        });
...
IWD.OPC.Shipping = {
    ...
    validateAddressForm: function (form) {
        ...
        var addressForm = new Validation('opc-address-form-shipping', {
            onSubmit: false,
            stopOnFirst: false,
            focusOnError: false
        });
...
IWD.OPC.Login = {
    ...
    init: function () {
        ...
        $j_opc(document).on('click', '.restore-account', function (e) {
            e.preventDefault();
            $j_opc('#login-form').hide();
            $j_opc('#login-button-set').hide();
            $j_opc('#form-validate-email').fadeIn();
            $j_opc('#forgotpassword-button-set').show();
        });
        ...
        $j_opc('#forgotpassword-button-set .back-link').click(function (e) {
            e.preventDefault();
            $j_opc('#form-validate-email').hide();
            $j_opc('#forgotpassword-button-set').hide();
            $j_opc('#login-form').fadeIn();
            $j_opc('#login-button-set').show();
...
IWD.OPC.Decorator = {
    ...
    setCurrentPaymentActive: function () {
        ...
        IWD.OPC.Decorator.decoratePayment($j_opc('#p_method_' + method).closest('dt'));
    },
    ...
    switchPaymentFieldsOrder: function (container) {
        $j_opc(container).find("input[name='payment[cc_number]'], #braintree_cc_number").each(function () {
    ...
    decoratePayment: function (parrent_dt) {
        ...
        if ($j_opc(container).find("input[name='payment[cc_number]']").hasClass('prepared') || $j_opc(container).find("#braintree_cc_number").hasClass('prepared'))
            return;
        ...
        $j_opc(container).find("#braintree_cc_number").closest('ul.form-list').addClass('opc-cc-container');
        ...
        $j_opc(container).find("#braintree_cc_cid").attr('placeholder', "123").after('<span class="cc-cid-placeholder"></span>');
        ...
        $j_opc(container).find("#braintree_cc_number").attr('placeholder', "4111 1111 1111 1111");
        $j_opc(container).find("input[name='payment[cc_number]']").attr('maxlength', "19");
        $j_opc(container).find("#braintree_cc_number").attr('maxlength', "19");

        // add class to decorate payment ONLY ONCE
        $j_opc(container).find("input[name='payment[cc_number]']").addClass('prepared');
        $j_opc(container).find("#braintree_cc_number").addClass('prepared');

        $j_opc(container).find("input[name='payment[cc_number]'], #braintree_cc_number").on('dblclick', function () {
            $j_opc(this).select();
        });

        $j_opc(container).find("input[name='payment[cc_number]'], #braintree_cc_number").on('input', function (e) {
...
ZipUpdater.prototype = {
    ...
    _setPostcodeOptional: function () {
        ...
        if (optionalZipCountries.indexOf(this.country) != -1) {
            while (this.zipElement.hasClassName('required')) {
                this.zipElement.removeClassName('required');
            }
